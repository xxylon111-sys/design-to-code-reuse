...
description: 在已有成熟前端代码库中还原设计稿（Pencil/Figma/截图）时使用。强制让 AI 先比对设计稿模块与现有组件库，优先复用线上组件代码而不是直接写新代码，并把每次用户的修正沉淀为项目级"设计特征 → 组件映射"知识库，让复用越用越准。**扩展说明：不仅复用组件，还优先复用项目中其他模块的视觉效果实现（渐变、毛玻璃、圆角面板、背景图等样式模式），同一效果不重复发明。**当用户提到「还原设计稿」「实现 Pencil 设计」「按这个设计图做页面」「把设计稿写成代码」「sketch 转代码」「图转代码」，且当前位于一个已有 src/components 或包含 design system 的成熟项目时，必须使用此 skill。即使用户没有明示"复用组件"，只要意图是把视觉稿落地到现有代码库，就应触发。
---
...
**视觉特效作为一等模块类型**：解析时对「背景图/渐变、阴影、圆角容器、边框描边、模糊/毛玻璃、动效过渡、配色组合」等纯视觉效果，也抽取为独立模块（role=`effect-*`），因为它们通常在项目其他页面已有成熟实现。例如「半透明白底+白边框的卡片容器」可能对应项目中的 `bg-prompt-make-bg border border-[#fff] rounded-[16px]` 模式。

### Step 3：组件与视觉特效复用比对（最关键）
...
对 role=`effect-*` 的模块以及任何模块的 features{} 中涉及的视觉样式，执行：

1. **关键字模式搜索**：提取视觉特征的关键词（如`半透明白底 卡片 渐变背景 白色边框`），用 grep_content 在项目中搜索匹配的 CSS class/Tailwind 组合/样式变量
2. **项目已有模块交叉引用**：用 Explore subagent 查「项目中哪些地方用到了类似的视觉效果」，特别关注设计稿所在项目内其他页面的实现 —— **同一项目内不同模块的相同视觉效果应优先保持一致**
3. **匹配 Tailwind 自定义工具类**：检查 tailwind.config 中的 `backgroundImage`、`colors`、`borderColor`、`boxShadow` 等扩展配置是否有直接匹配项
4. **匹配 CDN 背景图**：如果视觉特效涉及背景图片，搜索项目中已有的同类背景图 URL 模式

**视觉特效的三类结论**：

- `REUSE`：项目中有现成的 class/变量/背景图可直接使用（如 `bg-prompt-make-bg`、`border-[#fff]`、同一张 CDN 图）
- `EXTEND`：现有样式类接近但需微调（如已有圆角 `rounded-[12px]` 但设计稿要求 `rounded-[16px]`，直接改值即可）
- `NEW`：项目无现成实现，用 design tokens 新写

**重要**：视觉特效的 REUSE 优先级高于组件。例如「半透明白底卡片」如果项目中已有 `bg-prompt-make-bg border border-[#fff] rounded-[16px]` 这个实现模式，即使当前页面没有现成的 Card 组件，也应直接复用这个样式组合，而不是从零写 CSS。
...
- **视觉特效 REUSE**：对于视觉特效的复用，直接使用项目中其他模块已验证的 class 组合和样式值（如 `bg-prompt-make-bg border border-[#fff]`），**不要为了"看起来干净"而重新发明一套样式**。同一项目内相同视觉效果应该只有一份实现。

  > **反例**：项目中已有 `bg-prompt-make-bg`（半透明白色渐变背景），却在另一个页面写了 `bg-[linear-gradient(180deg,rgba(255,255,255,0.50)_0%,#FFF_7.83%)]` —— 效果相同但无法从项目层面统一维护。
  >
  > **正例**：直接复用 `bg-prompt-make-bg`，保持项目视觉效果一致性。
...
5. **效果复用先于样式重写**：设计稿中的视觉效果（渐变背景、毛玻璃卡片、阴影容器、圆角面板、背景图等）优先在项目其他模块中搜索同类实现，找到后直接复用其 class/变量/背景图 URL。**同一项目内相同视觉效果不应出现多份不同实现**。这比组件复用更基础 —— 即使没有现成的 React 组件，也应该先复用现成的样式组合。如果因为找不到同类实现而不得不新写，在 PR 中标注"项目内暂无此效果的现成实现，建议后续抽象为共享 token/class"。
